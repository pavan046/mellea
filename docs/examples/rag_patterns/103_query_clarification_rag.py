# pytest: skip_always
"""Retrieve-Clarify-Generate (RCG) — Query Clarification RAG.

Pipeline
--------
    question  ──►  rewrite  ──►  retrieve  ──►  clarify_query()
                                                   │
                                                   ├─ "CLEAR"          ──►  generate answer
                                                   └─ clarification    ──►  return question to user

Langflow equivalent
-------------------
    ChatInput → Memory → QueryRewrite → ElserRetriever → QueryClarification
                                                           → ConditionalRouter
                                                              → [GraniteBaseModel | ClarificationQuestion]

When to use
-----------
Use RCG when user queries may be ambiguous.  The clarification intrinsic checks
whether the retrieved documents can unambiguously answer the question.  If not, it
generates a clarifying question (e.g. "Do you mean A or B?") that you return to
the user.  This is the only RAG pattern that involves user interaction mid-pipeline.

Run
---
    uv run python docs/examples/rag_patterns/103_query_clarification_rag.py
"""
...
def run(question: str) -> None:
    ...
    # Step 4 — Conditional: generate or ask for clarification
    if clarification == "CLEAR":
        print("\n[4] Query is clear — generating answer...")
        ...
        # In a real application, you would return this to the user and wait
        # for their response before restarting the pipeline.
